...
try:
    ...
    from app.models.workflow.workflow import Workflow, Node, Connection, ExecutionRecord, NodeExecutionResult
...
ALL_MODELS = [
    ...
    User,
    # 租户域
    Organization, Team, Workspace, TeamMember,
    # 工作流域
    Workflow, Node, Connection, ExecutionRecord, NodeExecutionResult,
    # 应用域
    Application,
    # 对话域
...
def test_model_instantiation():
    ...
    def test_user_creation():
        ...
        return (
            isinstance(user.id, UUID) and
            user.name == "测试用户" and
            user.email == "test@example.com" and
            isinstance(user.created_at, datetime) and
            isinstance(user.updated_at, datetime)
        )
    ...
    def test_organization_creation():
        ...
        return (
            isinstance(org.id, UUID) and
            org.name == "测试企业" and
            isinstance(org.created_by, UUID) and
            isinstance(org.created_at, datetime)
        )
    ...
    def test_workflow_creation():
        ...
        return (
            isinstance(workflow.id, UUID) and
            workflow.name == "测试工作流" and
            isinstance(workflow.workspace_id, UUID) and
            isinstance(workflow.created_by, UUID)
        )
    ...
    def test_application_creation():
        ...
        return (
            isinstance(app.id, UUID) and
            app.name == "测试应用" and
            isinstance(app.workspace_id, UUID) and
            app.endpoint == "/api/test"
        )
    ...
    def test_subscription_creation():
        ...
        return (
            isinstance(subscription.id, UUID) and
            subscription.plan_type == "pro" and
            subscription.price == Decimal("99.00") and
            isinstance(subscription.quota_limits, dict)
        )
