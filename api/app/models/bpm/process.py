...
class ProcessDefinition(SQLModel, table=True):
    ...
    # 流程标识
    key: str = Field(index=True, max_length=100, description="流程唯一标识（如：workspace_approval）")
    ...
    process_config: dict = Field(default_factory=dict, sa_column=Column(JSON), description="流程配置（JSON）")
    
    # 节点定义
    nodes: list = Field(default_factory=list, sa_column=Column(JSON), description="流程节点列表")
    
    # 表单配置
    form_schema: Optional[dict] = Field(default=None, sa_column=Column(JSON), description="表单 Schema")
...
class ProcessInstance(SQLModel, table=True):
    ...
    # 业务关联
    business_key: Optional[str] = Field(default=None, max_length=255, index=True, description="业务键（如：application_id）")
    business_type: Optional[str] = Field(default=None, max_length=50, description="业务类型")
    
    # 关联 Workflow（如果从 Workflow 触发）
    workflow_run_id: Optional[UUID] = Field(default=None, index=True, description="关联的工作流执行ID")
