...
class Task(SQLModel, table=True):
    ...
    # 任务分配
    assignee: Optional[UUID] = Field(default=None, foreign_key="users.id", index=True, description="任务处理人")
    candidate_users: list = Field(default_factory=list, sa_column=Column(JSON), description="候选用户列表")
    candidate_groups: list = Field(default_factory=list, sa_column=Column(JSON), description="候选组列表")
