...
class Document(SQLModel, table=True):
    ...
    indexing_status: str = Field(max_length=50, index=True, default="waiting")  # waiting, parsing, splitting, indexing, completed, error
...
class DocumentSegment(SQLModel, table=True):
    ...
    disabled_at: Optional[datetime] = None
    disabled_by: Optional[UUID] = None  # Logical FK to users
    status: str = Field(max_length=50, index=True, default="waiting")  # waiting, indexing, completed, error
    error: Optional[str] = None
    created_by: UUID = Field(index=True)  # Logical FK to users
    created_at: datetime = Field(default_factory=datetime.utcnow, index=True)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
class DatasetApplicationJoin(SQLModel, table=True):
    """知识库应用关联表 - 多对多关系。
