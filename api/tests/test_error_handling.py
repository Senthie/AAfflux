...
class TestErrorHandling:
    ...
    def test_error_logging(self, client: TestClient, caplog):
        ...
        with caplog.at_level(logging.ERROR):
            ...
            if error_logs:
                assert any("404" in str(log.message) or "not found" in str(log.message).lower() for log in error_logs)
    ...
    def test_custom_exception_handling(self, client: TestClient):
        ...
        if response.status_code == 409:  # Conflict
            ...
            assert "already exists" in error_data["message"].lower() or "数据已存在" in error_data["message"]
    ...
    def test_sensitive_data_masking(self, client: TestClient):
        ...
        with patch('app.api.errors.logger') as mock_logger:
            # 触发一个简单的404错误，不涉及数据库
            response = client.get("/api/v1/nonexistent")
