...
class TestDataMigration:
    ...
    async def test_data_migration_backward_compatibility(self, migration_manager: DataMigrator, test_user: User, test_organization: Organization, test_session: AsyncSession):
        """测试数据迁移向后兼容性"""
        # 创建旧版本格式的工作流数据
    ...
    async def test_rollback_migration(self, migration_manager: DataMigrator, test_user: User, test_organization: Organization, test_session: AsyncSession):
        """测试迁移回滚"""
        # 创建新版本格式的工作流数据
    ...
    async def test_batch_migration(self, migration_manager: DataMigrator, test_user: User, test_organization: Organization, test_session: AsyncSession):
        """测试批量迁移"""
        # 创建多个需要迁移的工作流
        workflows = []
        for i in range(3):
    ...
    async def test_migration_history_tracking(self, migration_manager: DataMigrator, test_user: User, test_organization: Organization, test_session: AsyncSession):
        """测试迁移历史跟踪"""
        # 创建工作流
        workflow_definition = {
            "version": "1.0",
