...
class TestUserCRUD:
    ...
    async def test_read_user(self, test_session):
        ...
        # 读取用户
        result = await test_session.execute(select(UserEntity).where(UserEntity.id == user_id))
    ...
    async def test_read_user_by_email(self, test_session):
        ...
        result = await test_session.execute(
            select(UserEntity).where(UserEntity.email == 'email@example.com')
        )
        found_user = result.scalar_one_or_none()
    ...
    async def test_list_users(self, test_session):
        ...
        # 查询所有用户
        result = await test_session.execute(select(UserEntity))
    ...
    async def test_list_active_users_only(self, test_session):
        ...
        # 只查询未删除的用户
        result = await test_session.execute(select(UserEntity).where(not UserEntity.is_deleted))
