...
class TestFinalIntegration:
    ...
    async def test_all_modules_integration(
        self, client: TestClient, test_user: User, test_organization: Organization, auth_token: str
    ):
        """测试所有模块集成"""
        ...
        workflow_data = {
            ...
            'definition': {
                'nodes': [
                    {'id': 'start_node', 'type': 'start', 'config': {'message': 'Hello World'}},
                    {
                        'id': 'llm_node',
                        'type': 'llm',
        ...
        # 4. 发布应用
        publish_response = client.post(f'/api/v1/applications/{app_id}/publish', headers=headers)
        assert publish_response.status_code == 200

        # 5. 生成API密钥
        api_key_response = client.post(f'/api/v1/applications/{app_id}/api-key', headers=headers)
    ...
    async def test_database_crud_operations(
        ...
        # Update
        update_data = {'name': 'Updated CRUD Test Workflow', 'description': 'Updated description'}
    ...
    async def test_end_to_end_workflow_execution(
        ...
        workflow_data = {
            ...
            'definition': {
                'nodes': [
                    ...
                    {'id': 'output_node', 'type': 'output', 'config': {'format': 'json'}},
                ],
                'connections': [
        ...
        execution_response = client.post(
            f'/api/v1/workflows/{workflow_id}/execute', json=execution_data, headers=headers
        )
    ...
    async def test_performance_under_load(self, client: TestClient, auth_token: str):
        ...
        async def create_workflow(session, index):
            ...
            async with session.post(
                'http://testserver/api/v1/workflows', json=workflow_data, headers=headers
            ) as response:
    ...
    def test_api_documentation_completeness(self, client: TestClient):
        ...
        for endpoint in expected_endpoints:
            assert any(endpoint in path for path in paths.keys()), (
                f'Missing documentation for {endpoint}'
            )
